...
wezterm start --class=serv,serv bash -c "$HOME/.cargo/bin/chore ; ssh -p 773 spider@192.168.0.61" &
wezterm start --class=local,local bash -i -c "neofetch && python bin/archnews.py | sed -n '1,14 p' && $HOME/bin/p u; bash" &
